/**
 * @file TEST_SERVER.ts
 * @description ВРЕМЕННЫЙ ТЕСТОВЫЙ REAL-TIME WEBSOCKET + HTTP СЕРВЕР ДЛЯ ВВЭР-1200.
 * НАЗВАН КАПСОМ ДЛЯ БЫСТРОГО УДАЛЕНИЯ ПОСЛЕ ТЕСТИРОВАНИЯ.
 */
...
// Актуальные органы управления реактором со всеми расширенными параметрами
const currentInputs: ControlInputs = {
...
// 1. Создаем HTTP сервер, отдающий тестовую веб-панель TEST_DASHBOARD.html
...
// 2. Создаем WebSocket сервер поверх HTTP сервера
const wss = new WebSocketServer({ server });

wss.on('connection', (ws: WebSocket) => {
  console.log('[TEST_SERVER] Клиент подключился к телеметрии (Unity / React / SCADA)');
    ...
    try {
      ...
      switch (cmd.action) {
        ...
        case 'TRIP_AZ':
          ...
          console.log('[TEST_SERVER] ВНИМАНИЕ: СРАБОТАЛА АВАРИЙНАЯ ЗАЩИТА АЗ-1 (РУЧНОЙ СБРОС СТЕРЖНЕЙ)!');
          break;
        ...
        case 'TOGGLE_RCP': {
          ...
          if (pumpIdx >= 0 && pumpIdx < 4) {
            ...
            console.log(`[TEST_SERVER] ГЦН-${pumpIdx + 1}: ${currentInputs.rcpActive[pumpIdx] ? 'ВКЛ' : 'ОТКЛ'}`);
          }
          break;
        }
        case 'SET_RCP_SPEED':
        ...
        // Турбоустановка и редукционные установки
        ...
        case 'TRIP_TURBINE':
          ...
          console.log(`[TEST_SERVER] Стопорные клапаны турбины (СК ТП): ${currentInputs.turbineStopValvesOpen ? 'ОТКРЫТЫ' : 'ПОСАЖЕНЫ (ТРИП ТУРБИНЫ)'}`);
          break;
        case 'SET_BRU_K':
        ...
        case 'RESET':
          currentInputs.manualScram = false;
          currentInputs.rodPositionTarget = 0.70;
          currentInputs.turbineValveTarget = 1.00;
          currentInputs.turbineStopValvesOpen = true;
          currentInputs.boronInjectionRate = 0.0;
          currentInputs.boronPumpActive = false;
          currentInputs.pureWaterPumpActive = false;
          currentInputs.rcpActive = [true, true, true, true];
          currentInputs.rcpSpeedTarget = 1.0;
          currentInputs.pzrControlMode = 'AUTO';
          currentInputs.feedwaterControlMode = 'AUTO';
          currentInputs.bruKTarget = -1;
          currentInputs.bruATarget = -1;
          currentInputs.gridConnected = true;
          engine.reset();
          console.log('[TEST_SERVER] Энергоблок сброшен в исходный стационарный режим 100%');
          break;
      }
    } catch (e) {
      console.error('[TEST_SERVER] Ошибка разбора команды от клиента:', e);
    }
  });

  ws.on('close', () => {
    console.log('[TEST_SERVER] Клиент отключился');
  });
});

// 3. Главный симуляционный цикл Real-Time (20 кадров в секунду: dt = 0.05 с = 50 мс)
const DT = 0.05;
setInterval(() => {
  const state = engine.step(DT, currentInputs);

  // Формируем полный структурированный JSON-пакет телеметрии для Unity 3D и веб-пульта
  const telemetry = {
    type: 'TELEMETRY',
    ...
    // 29 Сигнальных Ламп / Табло БЩУ (для Unity и SCADA)
...
server.listen(PORT, () => {
  console.log('========================================================================');
  console.log(`  [TEST_SERVER] ВРЕМЕННЫЙ ТЕСТОВЫЙ СЕРВЕР ЗАПУЩЕН!`);
  console.log(`  Веб-интерфейс оператора: http://localhost:${PORT}`);
  console.log(`  WebSocket стриминг телеметрии (20 Гц): ws://localhost:${PORT}`);
  console.log('========================================================================\n');
});
